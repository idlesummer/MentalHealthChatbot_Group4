...
export const INTENT_COMPLETION_REGISTRY: Record<Intent, string> = {
  I1: 'Complete only if the user described a clear situation or event that triggered distress AND at least ONE contextual detail (e.g., what happened, when or where it occurred, who was involved, or why it mattered).',
  I2: 'Complete if the user expressed an automatic thought - an immediate, self-referential interpretation or belief that arose from the situation (e.g., \'I\'m not good enough\', \'They must hate me\'). feelings alone usually do not count unless a belief is clearly implied.',
