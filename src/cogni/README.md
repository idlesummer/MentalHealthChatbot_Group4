...
A standalone, reusable CBT (Cognitive Behavioral Therapy) pipeline implementation with support for 8-stage intent-driven workflow, cognitive distortion detection, and multiple prompt engineering techniques.
...
✅ **Cognitive Distortion Detection** - Identifies 10 cognitive distortions
...
console.log(result.cognitiveDistortion); // Identified distortion
```
...
```typescript
{
  ...
  currentIntent: string;        // Current CBT stage (I1-I8)
  conversationHistory: Message[]; // Full conversation history
  promptTechnique: PromptTechnique; // Selected prompting technique
}
```
...
```typescript
{
  ...
  cognitiveDistortion?: {       // Identified distortion
    distortion: string;
    confidence: number;
    rationale: string;
  };
...
##### `identifyCognitiveDistortions(message: string): Promise<CognitiveDistortionResult>`
...
const stage1 = await engine.generateResponse({
...
const stage2 = await engine.generateResponse({
...
export async function generateResponse(
  ...
  const result = await cogniEngine.generateResponse({
    message,
    currentIntent: intent,
    conversationHistory: messages,
    promptTechnique: prompt,
  });
...
interface CognitiveDistortionResult {
